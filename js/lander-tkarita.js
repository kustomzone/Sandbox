...
GameState.prototype.create = function() {
    ...
    this.ACCELERATION   = 294; // pixels/second/second
    this.MAX_SPEED      = 3432;// pixels/second
    this.DRAG           = 0;   // pixels/second
    this.GRAVITY        = 98;  // pixels/second/second
    ...
    this.ship.yOrig = -1000000 + this.ship.y;
    this.ship.yChange = 0;
    ...
    // world
    this.world.setBounds(0, -25000, 1440, 1000000);
...
GameState.prototype.update = function() {
    ...
    this.cameraYMin = this.ship.y - this.game.height - 1000000;
    this.camera.y = this.cameraYMin; // custom follow style (needed?)
    ...
    */
    ...
    if (onTheGround) {
        if (Math.abs(this.ship.body.velocity.y) > 20 || Math.abs(this.ship.body.velocity.x) > 30 || Math.abs(this.ship.angle + 90) > 15) {
...
GameState.prototype.render = function() {
    ...
    this.ship.altitude = Math.abs(Math.floor(this.ship.y)-752);
	 ...
	 var am = this.ship.altitude / 10;
	 var ak = am / 1000;
	 
    this.game.debug.text(am.toFixed(1) + " m", 175, 14, "#00D000"); // ship altitude
	 
	 var vx = this.ship.body.velocity.x;
	 var vy = this.ship.body.velocity.y;
	 
	 var vd = Math.floor(Math.hypot(vx, vy));
	 
	 var vm = vd / 10;
	 
	 var vc = (vy > 0 ? "#dd0" : "#0d0");
	 
	 if (Math.abs(vx) > 30) vc = "#d00";
	 if (vy > 20) vc = "#d00";
	 
    this.game.debug.text((vy > 0 ? "-" : "+") + vm.toFixed(1) + " m/s", 350, 14, vc); // ship velocity
...
//  }
    ...
    if ((this.fuelGuage.scale.x < 0.175) && (this.lives)) { this.game.debug.text('low fuel', 53, 14, "#FF0000"); }
	 ...
	 var vmax = this.MAX_SPEED * (1 - ak / 14 / 6);
	 var mach = vd / vmax;
	 
	 var drag = (70 - ak) / 70 * vm * (1 + (vm - 1) * mach) / 16;
	 
	 if (drag < 0) drag = 0;
		 
	 this.ship.body.maxVelocity.setTo(vmax * 10, vmax * 10);
	 this.ship.body.drag.setTo(drag, drag);
	 
	 if (vd > vmax) this.game.debug.text("Mach " + mach.toFixed(1), 525, 14, "#0DD");
};
