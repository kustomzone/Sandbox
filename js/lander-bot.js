...
// Reusable Rocket Lander (Sandbox v0.04) by Andrew Kidoo (bot@zeroid.bit)
...
GameState.prototype.create = function() {
	...
	this.ship.xOrig = -2874;
    this.ship.yOrig = -25000;
	this.ship.xChange = 0;
    this.ship.yChange = 0;
    ...
    this.hero.scale.x /= 1.35;
    this.hero.scale.y /= 1.35;
    ...
    // 2nd player..
    // this.player = this.game.add.sprite(this.game.world.centerX, this.game.world.centerY, this.hero);
    ...
    // world
    this.world.setBounds(-1440, -25000, 1440, 25000);
    ...
    */
	...
	/*
	// TODO: change to 1 graphic, not tons of sprites...
	this.groundBmp = this.game.make.bitmapData(0,0);
    this.groundBmp.ctx.rect(0, 0, 4320, 11);
    this.groundBmp.ctx.fillStyle = "red";
    this.groundBmp.ctx.fill();
	this.solidGround = this.game.add.sprite(-2880, this.game.height - 32, this.groundBmp);
	this.game.physics.enable(this.solidGround, Phaser.Physics.ARCADE);
	this.solidGround.body.immovable = true;
    this.solidGround.body.allowGravity = false;
    this.ground.add(solidGround);
	*/
    
    for (var x = -2880; x < 1440; x += 32) {
        // add the ground blocks, enable physics on each, make them immovable
        var groundBlock = this.game.add.sprite(x, this.game.height - 32, 'ground', this.ground);
    ...
    this.stars = [];
    for (var i = 0; i < 500; i++) {
        this.star = { x: Math.floor(Math.random() * 4320) - 2880, y: Math.floor(Math.random() * 26000) - 25500};
    ...
    this.mountain = this.game.make.sprite(this.game.world.centerX, this.game.world.centerY + 244, mnt.generateTexture(), 0, this.group);
...
GameState.prototype.drawTerrain = function() {
	...
	this.procMount.lineTo(i * 43 + 20, this.terrain[(this.count + i) % this.steps] + 580);
    this.procMount.lineTo(i * 43 + 20, 765);
    this.procMount.lineTo(0, 765);
    this.procMount.endFill();
    
    // todo: move Bmp than redraw..
    this.procMountain1 = this.game.add.sprite(-2880, 535, this.procMount.generateTexture());
	this.procMountain2 = this.game.add.sprite(-1440, 535, this.procMount.generateTexture());
	this.procMountain3 = this.game.add.sprite(    0, 535, this.procMount.generateTexture());
...
GameState.prototype.drawStars = function() {
    ...
    for (var i = 0; i < this.stars.length; i++) { 
        this.starPoint = this.game.make.bitmapData(4, 4);
        this.starPoint.ctx.rect(0, 0, this.rnd.integerInRange(1, 3), this.rnd.integerInRange(1,3));
...
GameState.prototype.makePlatforms = function() {
    ...
    // this.platforms.createMultiple(7, this.platformBmp);
...
GameState.prototype.moveHero = function() {
...
GameState.prototype.update = function() {
    ...
    // so track the maximum amount that the ship has travelled
	this.ship.xChange = Math.max(this.ship.xChange, Math.abs(this.ship.x - this.ship.xOrig));
    this.ship.yChange = Math.max(this.ship.yChange, Math.abs(this.ship.y - this.ship.yOrig));
    
    // change.. hero/ship
    this.world.setBounds(-this.ship.xChange + this.game.width, -this.ship.yChange, this.ship.xChange, this.game.height + this.ship.yChange);
    
    // custom follow style
	this.cameraXMin = this.ship.x;
    this.cameraYMin = this.ship.y - 25000;
    ...
    */
	...
	// collide the ship with the mountain
    // this.game.physics.arcade.collide(this.procMountain1, this.ground);
	// this.game.physics.arcade.collide(this.procMountain2, this.ground);
	// this.game.physics.arcade.collide(this.procMountain3, this.ground);
    
	// some animation sound
    // if (this.procMountain1.body.touching.down && !this.mountIsDown) { this.explosionSound.play(); this.mountIsDown = true; }
    ...
    // Paused: collide the ship with the mountain
    // this.game.physics.arcade.collide(this.ship, this.mountain);
    
    // Todo: Side scroller..
    // keep the ship in world
    if (this.ship.x > this.game.width) { this.ship.x = -2880; }
    if (this.ship.x < -2880) { this.ship.x = this.game.width; }
...
GameState.prototype.render = function() {
    ...
    this.ship.xPos = Math.floor(this.ship.x / 10);
	this.ship.altitude = Math.abs(Math.floor((this.ship.y - 752) / 10));
	this.game.debug.text(this.ship.xPos || '--', 370, 14, "#D0D000"); // ship X pos
    this.game.debug.text(this.ship.altitude || '--', 430, 14, "#00D000"); // ship altitude
